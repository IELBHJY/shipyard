...
public class Data {
    ...
    public final static double L=500;
    //个数
    public int n;
    public int t;
    public int r;
    ...
    public double[][] w;
    public int[][] prior;
    public int[] s;//task i is super task?
    ...
    public void readData(String filePath) throws IOException, BiffException {
        try {
            ...
            Sheet sheet = rwb.getSheet("car"+Parameter.T);
            //行数(表头的目录不需要，从1开始)
            Cell cell=null;
            for(int i=1; i<t; i++){
            ...
            sheet=rwb.getSheet("task"+Parameter.N);
            for(int i = 1; i<n; i++){
                cell=sheet.getCell(0,i);
                String temp=cell.getContents();
    ...
    private void calTwoTasksTime(){
        ...
        for(int i=1;i<n;i++){
            w[0][i]=calPathTime("车场",taskStartPlace[i])/5;
            w[i][0]=w[0][i];
        }
    }
    ...
    public void calSuperTask(){
        int max=0;
        for(int i=1;i<t;i++){
            if(truckCaptitys[i]>max){
                max=truckCaptitys[i];
            }
        }
        for(int i=1;i<n;i++){
            if(taskWeights[i]>max){
                s[i]=1;
                Parameter.superNum++;
            }else{
                s[i]=0;
